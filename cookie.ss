...
(define (cookie-unescape str)
  (regexp-replace* #px"#3B" (regexp-replace* #px"#2C" str ",") ";"))
...
(define (cookie-string? s (value? #t))
  (unless (string? s)
    (raise (make-cookie-error (format "String expected, received: ~a" s)
...
(define (expires->rfc822-string seconds)
  (date->string (time-utc->date (make-time time-utc 0 seconds) 0)
                "~a, ~d-~b-~Y ~H:~M:~S GMT"))
